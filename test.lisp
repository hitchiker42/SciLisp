...
(defun abs-diff (x y) (abs (- (abs x) (abs y))))
...
(assert-fp-equal (sin pi) 0.0)
(assert-fp-equal (tan pi) 0.0)
(assert-eq (logandn 1337 17) 1320)
(assert-eq (logxor 1337 7) 1342)
(assert-eq (logior 1337 7) 1343)
(assert-eq (logand 1337 17) 17)
(assert-eq (twos-compliment 1337) -1337)
(exp e)
(expt 4 4)
(expt e Meps)
(abs -21.9)
...
(rand-float)
(rand-float nil 1000)
(setq state (seed-rand-r))
(rand-int state)
(rand-int state #t)
(rand-float state)
(rand-float state 1000)
(aref arr-iota 4)
(length ls)
(length arr)
(consp arr)
...
(let ((x 77))
  (progn
    ...
    (assert-eq x 77)))
(assert-eq x 0)
(let ((ls (merge-sort (rand-list 20) >)))
  (assert (is-sorted ls >)))
