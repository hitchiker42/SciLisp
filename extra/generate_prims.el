...
; the sig field here will just be the list of arguments
; it'll expand to (<funname>':' <arglist>)
; ':' will work in signatures since it's a reserved character
;i.e. there can't be a function 'cons:' without quoting the ':'
;for functions with no arguments no sig needs to be specified as
...
(define basic-prims-list
  '(("!=" "lisp_numne" 2 :sig "(num1 num2)")
    ...
    ("addhash" "hashtable_add_entry" 3 :optargs 1 :sig "(hash-table key value &optional option)")
    ...
    ("bigfloat" "lisp_bigfloat" 1 :optargs 2 :sig "(number-or-string &optional prec rnd)")
    ("bigint" "lisp_bigint" 1 :sig "(number)")
    ("c-ptr-val" "lisp_dereference_c_ptr" 1 :sig "(pointer)")
    ("cat" "lisp_cat" 0 :restarg 1 :sig "(&rest seqs)")
    ("ccall" "ffi_ccall" 5 :optargs 1 :sig "(function-name libname return-type argtypes args &optional thread)")
    ("copy-cons" "copy_cons" 1 :sig "(cell)")
;    ("ffi-ccall" "ffi_ccall" 5 :optargs 1 :sig "(function-name libname return-type argtypes args &optional thread)")
    ...
    ("gt" "lisp_cmp_gt" 2 :sig "(num1 num2)");("eq" "lisp_cmp_eq" 2)
    ("hash-table-entries" "hashtable_num_entries" 1 :sig "(hash-table)")
    ("hash-table-growth-factor" "hashtable_growth_threshold" 1 :sig "(hash-table)")
    ("hash-table-growth-size" "hashtable_growth_factor" 1 :sig "(hash-table)")
